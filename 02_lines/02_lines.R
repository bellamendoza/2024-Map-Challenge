...
# Identify the unique states from shakira data
shakira_states <- unique(shakira_data$state)
...
ggplot(data = us_states) +
  ...
  geom_sf(data = points_sf, aes(fill = name), shape = 21, color = "black", fill="#fc9003",  size = 2, stroke = 0.5) +  # Points
  geom_sf(data = line_sf, color = "#fc9003", size = 1) +
  ...
  coord_sf(xlim = c(-125, -65), ylim = c(25, 50), label_axes = "----", label_graticule = "----") +
